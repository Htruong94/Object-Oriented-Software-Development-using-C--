...
void loadData(const char* filename, sdds::Autoshop& as)
{
	...
	while (file)
	{
		...
		try {
			...
			std::cerr << "Unrecognized record type: [" << cherr << "]" << std::endl;
		}
...
int main(int argc, char** argv)
{
	...
	std::list<const sdds::Vehicle*> vehicles;
	{
		...
		auto fastVehicles = [](const sdds::Vehicle* veh) { //if top speed of vehicle is greater than 300, return true
			if (veh->topSpeed() > 300) {
				return true;
			}
			else {
	...
	std::cout << std::endl;
	{
		...
		auto brokenVehicles = [](const sdds::Vehicle* veh) { //if the vehicle has a conditionm of "b",  return true
			if (veh->condition() == "b"){
				return true;
			}
			else {
